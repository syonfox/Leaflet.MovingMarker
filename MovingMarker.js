...
/**
 *
 * Creates a new MovingMarker object which extends L.Marker.
 * It animates the movement of the marker along a given set of coordinates (latlngs)
 * with a corresponding set of durations (durations) between each coordinate.
 * @class L.Marker.MovingMarker
 * @extends L.Marker
 * @param {L.LatLng[]} latlngs - Array of LatLng objects representing the path that the marker should follow.
 * @param {number[]} durations - Array of durations (in milliseconds) representing the time that the marker should take to move from one coordinate to the next.
 * @param {Object} options - Options for the MovingMarker object.
 * @param {boolean} [options.autostart=false] - Whether to start the animation automatically after the object is created.
 * @param {boolean} [options.loop=false] - Whether to loop the animation back to the start after reaching the end.
 **/
L.Marker.MovingMarker = L.Marker.extend({
    ...
    options: {
        ...
        // rotationOrigin: "center", // dont override the other defaults
    ...
    _animate: function (timestamp, noRequestAnim) {
        ...
        if (this.options.center) {
            if (this.options.center === 'lock') {
                this._map.setView(this._latlng, undefined, true)
            } else {
